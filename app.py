...
"resolution": raw_config.get("resolution") or ("720p" if "fast" in (raw_config.get("video_model") or settings["default_video_model"]) else "1080p"),
        ...
        # Smart duration: null/0 means let the planner decide from story length.
        "target_duration": clamp_int(raw_config.get("target_duration"), 30, 8, 180) if raw_config.get("target_duration") else None,
...
def estimate_job_cost(config, settings, character_count=None, planner_usage=None, actual_duration=None):
    """
...
def run_job_async(job_id, user_id, config, settings, storyboard_text, storyboard_path, estimated_credits):
    ...
    try:
        ...
        character_count = len(planner.get("characters", {}))
        actual_duration = sum(int(s.get("duration", 5)) for s in planner.get("shots", [])) if not config.get("target_duration") else None
        estimated_credits, _provider = estimate_job_cost(config, settings, character_count=character_count, planner_usage=planner.get("usage"), actual_duration=actual_duration)
...
def journal():
    ...
    if not user:
        return redirect("/")
    return render_template("inner.html", open_panel="journal")
